...
# Function to generate embeddings for the input text using the model
def generate_embedding(text: str) -> list:
    embeddings = bedrock_embeddings.embed_query(text)
    return embeddings


# Define the incoming message structure using Pydantic
class MessageInput(BaseModel):
    type: str  # Expected values: 'human' or 'ai'
    text: str
    timestamp: Optional[str] = None  # ISO formatted timestamp is optional


# Class representing a message with metadata and embedding info
class Message:
    def __init__(
        self,
        user_id: str,
        ...
        self.type = type  # Expected: 'human' or 'ai'
        self.text = text
        # Use current UTC time if timestamp is not provided
        self.timestamp = timestamp or datetime.datetime.now(datetime.timezone.utc)
        # Generate text embeddings
        self.embeddings = generate_embedding(text)

    # Convert the message instance to a dictionary for database insertion
    def to_dict(self):
        return {
...
@app.post("/conversation/{user_id}/{conversation_id}")
def add_message(user_id: str, conversation_id: str, message: MessageInput):
    # Validate that the message type is either 'human' or 'ai'
    if message.type not in ("human", "ai"):
        raise HTTPException(status_code=400, detail="Type must be 'human' or 'ai'")

    # Convert timestamp from ISO format if provided
    timestamp = (
    ...
    new_message = Message(
        user_id, conversation_id, message.type, message.text, timestamp
    )

    # Insert the message document into the MongoDB collection
...
# Endpoint to fetch conversation messages by conversation ID and user ID
@app.get("/conversation/")
async def conversation(user_id: str, conversation_id: str):
    """
    Retrieves all conversation records for the given user_id and conversation_id.
    """
    try:
        # Query messages and exclude unnecessary fields
        documents = list(
...
async def generate_conversation_summary(documents: str):
    ...
    try:
        ...
        prompt = (
            ...
            f"Input JSON: {json.dumps(documents)}"
        )
...
async def retrieve_memory(user_id: str, text: str):
    ...
    try:
        ...
        return {"memory_items": memory_items, "context": context, "summary": summary}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
